"""Specialized ADK agent for spending insights tool orchestration."""
...
class InsightsAgentInput(BaseModel):
    """Validated input contract for `InsightsAgent` execution."""

    statement_uuid: UUID


class InsightsAgent(WalletMindBaseAgent):
    """ADK-backed specialized agent that executes WalletMind insights tool only."""

    def __init__(
        self,
        *,
        tool: FunctionTool = INSIGHTS_TOOL,
        ...
        super().__init__(
            metadata=AgentMetadata(
                name="insights_agent",
                description=(
                    "Specialized WalletMind agent that executes spending insights "
                    "analysis through the ADK InsightsTool."
                ),
                tags=("insights", "spending", "analytics"),
            ),
            logger=logger,
        )
        self._tool = tool
    ...
    def validate(self, *, context: AgentExecutionContext) -> None:
        """Validate context envelope and insights input payload."""

        super().validate(context=context)
        InsightsAgentInput.model_validate(context.extras or {})
    ...
    async def execute_impl(self, *, context: AgentExecutionContext) -> Any:
        ...
        payload = InsightsAgentInput.model_validate(context.extras or {})
